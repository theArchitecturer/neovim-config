...
function! s:system_cut(type, ...) abort
  let mode = <SID>resolve_mode(a:type, a:0)
  let unnamed = @@
  if mode == s:linewise
    let lines = { 'start': line("'["), 'end': line("']") }
  ...
  silent call system(command, getreg('@'))
  if g:system_copy_silent == 0
    echohl String | echon 'Cut to clipboard using: ' . command | echohl None
  endif
  let @@ = unnamed
endfunction
...
xnoremap <silent> <Plug>SystemCut :<C-U>call <SID>system_cut(visualmode(),visualmode() ==# 'V' ? 1 : 0)<CR>
nnoremap <silent> <Plug>SystemCut :<C-U>set opfunc=<SID>system_cut<CR>g@
nnoremap <silent> <Plug>SystemCutLine :<C-U>set opfunc=<SID>system_cut<Bar>exe 'norm! 'v:count1.'g@_'<CR>
...
if !hasmapto('<Plug>SystemCut', 'n') || maparg('cx', 'n') ==# ''
  nmap cx <Plug>SystemCut
endif

if !hasmapto('<Plug>SystemCut', 'v') || maparg('cx', 'v') ==# ''
  xmap cx <Plug>SystemCut
endif

if !hasmapto('<Plug>SystemCutLine', 'n') || maparg('cX', 'n') ==# ''
  nmap cX <Plug>SystemCutLine
endif
" vim:ts=2:sw=2:sts=2
